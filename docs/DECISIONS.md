...
If `ENABLE_WEB_CLIENT=true` and a telephony provider (e.g. ACS) is detected as configured, `server.py` must fail startup (log and `sys.exit(1)`, the same pattern `BridgeConfigError` already uses), not merely log a warning and continue running with both active. **Supersedes** the "warn-only" behavior D-005/D-024/U05 shipped, and answers Q-007.
