...
**Why:** the web debug client (`/web/ws`) is unauthenticated by design (D-007) and was never meant to coexist with a real customer-facing phone line — a live, unauthenticated debug backdoor sitting next to production telephony is not an acceptable risk for the pilot, even temporarily, and the founder chose fail-closed over fail-open when asked directly. **Source:** founder, answering Q-007 (status-only PR, 2026-09-27). To be implemented in U09 (Task 8 — bridge calls), since that unit already touches `server.py`'s startup/routing wiring; see STATUS.md §1.
