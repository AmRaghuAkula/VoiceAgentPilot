...
### D-024 · 2026-09-25 · Server startup wiring: web client's static folder is conditional too, and `BridgeConfig` never reprs its token
`server.py` builds `app = Quart(__name__, static_folder="static" if bridge.enable_web_client else None)`, not just a conditional `/web/ws` and `/` route registration. `BridgeConfig.media_ws_token` (`app/bridge_config.py`) is declared `field(repr=False)`.
**Why:** U05's first Opus review round found that disabling the web client by unregistering `/web/ws` and `/` (as D-007/the plan's literal instructions specified) left Quart's own default static route still serving `/static/index.html` and `/static/audio-processor.js` unauthenticated regardless of `ENABLE_WEB_CLIENT` — the debug client's files were still publicly readable even though its entry points were gone. The `cso` review separately found that once `BridgeConfig` is stored in `app.config["BRIDGE"]` (this unit's own change), it becomes reachable from anywhere in the app, and its auto-generated dataclass repr included `media_ws_token` in full — a latent leak via any future `logger.debug(app.config)`, `logger.info(bridge)`, or a traceback that captures local variables. **Source:** U05's Opus code review round 1 (static-folder gap) and `cso` round 1 (repr leak).

### D-025 · 2026-09-25 · Open items from U05, deferred to the partner rather than fixed in-unit
U05's reviews raised five items that were not fixed in that unit, because fixing them would mean deviating from the plan's literal prescribed code, touching an unmodified upstream file (D-016), or making a design call beyond Task 4's stated interface:
- `acs_active` for `load_bridge_config()` is `bool(os.getenv("ACS_CONNECTION_STRING"))`, not `detect_provider() == "acs"` — matches the plan's literal instruction text; a whitespace-only `ACS_CONNECTION_STRING` would trip this check True while `bridge_config._get()` treats it as unset elsewhere.
- `MAX_CONCURRENT_CALLS` and `CALL_IDLE_TIMEOUT` still use raw, unvalidated `int(os.getenv(...))` in `server.py` — out of Task 4's stated interface (only the endpoint, `BRIDGE`, `VOICE_LIVE_API_VERSION`, `INTERIM_RESPONSE` and `max_call_seconds` were in scope).
- `app/config_validator.py`'s missing-endpoint error message names only `AZURE_VOICE_LIVE_ENDPOINT`, even when the spec's `VOICE_LIVE_ENDPOINT` name (D-003) was the one left unset — that file is unmodified upstream code (`git diff upstream/main` on it is empty), out of scope per D-016.
- Nothing in code stops `ENABLE_WEB_CLIENT=true` and an active ACS provider from running at the same time. D-007 says the web client is "never set in a deployed environment," but today that's enforced only by the env var, not by the bridge refusing to start. Worth deciding before the M6 deploy: warn-only (today's behavior) vs. refuse to start.
- Upstream's own `app.run(host="0.0.0.0", debug=_debug)` bind-all-interfaces/debug-mode behavior — unmodified upstream code, out of scope; noted here for the eventual production security review.
**Why:** to make sure these don't get silently lost between sessions. **Source:** U05's Opus code review (4 rounds) and `cso` (2 rounds), PR #6.

### D-026 · 2026-09-26 · Agent-mode `interim_response` must be deep-thawed before it reaches Voice Live
`VoiceLiveMediaHandler._agent_session_config()` (`app/handler/voicelive_media_handler.py`) passes `self.interim_response` through a new recursive `_deep_thaw()` helper (`MappingProxyType` → `dict`, `tuple`/`list` → `list`) before building the agent-mode `RequestSession`, instead of a shallow `dict(...)` copy.
**Why:** `BridgeConfig`'s `_deep_freeze()` (D-023) freezes nested dicts/lists too, so `INTERIM_RESPONSE_JSON` as loaded by the real config path can carry `MappingProxyType`/`tuple` nested inside a top-level dict. A shallow `dict(self.interim_response)` copy left those nested values frozen, and `json.dumps()` on the session payload sent to Voice Live would raise `TypeError: mappingproxy is not JSON serializable` for any agent-mode call whose `INTERIM_RESPONSE_JSON` had a nested object — a connection-breaking bug in every such call, not a cosmetic one. Two independent Opus reviews (code-review and `cso`) converged on this same root cause in round 1 of U07's review. Fixed with a regression test that builds the value via the real `_deep_freeze()` and asserts `json.dumps()` succeeds. The same U07 fix round also added defense-in-depth: `AgentRoute.__post_init__` (`app/routing.py`) now enforces the pinned-version format independently of `bridge_config`'s own check, `_voicelive_connected` is set only after `session.update`/`response.create` succeed, `on_voicelive_ended()` is called under try/except in the receiver's `finally`, and `stop_forwarding_agent_audio()` now drops already-buffered TTS audio, not just future deltas. **Source:** U07 Opus code-review + `cso`, round 1 findings (both independently found the same bug), PR #8.

### D-027 · 2026-09-27 · `CallSession._safe_hang_up()` must not treat a cancelled or transient hang-up attempt as a permanent no-op
Three real defects were found and fixed in the call-ending path (`app/providers/acs/call_session.py`) implementing D-005's single idempotent `request_end()`:
- (#A) A late `on_play_done()` (or a `play_media` failure) could cancel the play-safety timer while its own spawned `_safe_hang_up()` was already awaiting the ACS hang-up HTTP call. `CancelledError` isn't caught by `_safe_hang_up`'s `except Exception`, so the request was silently dropped, and the follow-up retry no-op'd because `_hung_up` was already `True` — the call stayed up with no further attempt to end it. Fixed: timer cancellation now goes through `_cancel_safety_timer()`, which only cancels the timer while it is still merely sleeping (guarded by `not self._hung_up`); the underlying hang-up call is also wrapped in `asyncio.shield()` as defense in depth.
- (#D) `request_end()` can fire before `set_answered()` supplies a `call_connection_id` (e.g. a route-miss `CallConnected` racing ahead of `answer_call()`'s own response). `_safe_hang_up()` used to silently no-op in that case with nothing to ever retry it, leaving the caller connected in silence indefinitely once `wait_timeout` elapsed. Fixed: a new `_hangup_pending_answer` flag records the deferred hang-up, and `set_answered()` finishes it once the connection id is known.
- (#B) Any `HttpResponseError` on hang-up, including a transient 5xx/429, was treated the same as a terminal "call already gone" error, permanently setting `_hung_up` and blocking any future retry. Fixed: only a narrow `{404, 410}` status set is now treated as terminal; anything else clears `_hung_up` so a later call (another end path, or `sweep()`'s new `force_hang_up()`) can retry.
**Why:** this is the unit D-005 and the four design-spec review rounds (D-008) exist to protect — "every way a call ends" must actually end it, not leave the caller connected in silence under a race or a transient failure. Two independent Opus review rounds (code-review + `cso`) confirmed the fixes close the races without introducing a new one, and both reconfirmed `request_end()`'s idempotency, that no double hang-up is possible, that only one Voice Live close task is ever spawned, and that no session is left permanently stuck in the registry (`sweep()` is the backstop either way). A residual, lower-severity gap — transient failures are no longer *permanently* blocked but aren't retried *promptly* either, and the ACS `hang_up()` call itself has no timeout — was logged as Q-010 rather than fixed in this unit, to avoid a third fix-and-re-review cycle on an already twice-reviewed unit. **Source:** U08 Opus code-review, round 1 findings (#A/#B/#D) and round 2 confirmation ("ready to merge"); `cso` round 1 and round 2 both PASS. PR #9.

### D-028 · 2026-09-27 · Web debug client and a telephony provider can never both be active — the bridge refuses to start, it does not just warn
If `ENABLE_WEB_CLIENT=true` and a telephony provider (e.g. ACS) is detected as configured, `server.py` must fail startup (log and `sys.exit(1)`, the same pattern `BridgeConfigError` already uses), not merely log a warning and continue running with both active. **Supersedes** the "warn-only" behavior D-007/D-024/D-025 shipped, and answers Q-007.
**Why:** the web debug client (`/web/ws`) is unauthenticated by design (D-007) and was never meant to coexist with a real customer-facing phone line — a live, unauthenticated debug backdoor sitting next to production telephony is not an acceptable risk for the pilot, even temporarily, and the founder chose fail-closed over fail-open when asked directly. **Source:** founder, answering Q-007 (status-only PR, 2026-09-27). **Implementation note (corrects an earlier plan on this same day):** originally slated for "whichever M4 unit touches `server.py`," but on checking the actual plan text (`docs/superpowers/plans/2026-09-25-telephony-bridge-step3.md`), no unit from Task 8 through Task 11 modifies `server.py` at all — only Task 4 (U05, already merged) does. Rather than bolt an out-of-scope change onto a unit whose stated interface doesn't include it, D-028 gets its own small standalone unit, inserted as **U09** (between U08 and the renumbered U10–U13; see STATUS.md §1 and the milestones doc's M3 section) rather than folded into any existing task.

### D-029 · 2026-09-27 · M6 (deploy) is unblocked ahead of the ACS phone number itself (Q-002 stays open)
M6 no longer waits on Q-002 (buying/receiving the ACS test number). The Container App, Key Vault and Event Grid subscription can be stood up against `rg-hireastra-voice-pilot`/`hireastra-resource` now, with `az login`/`azd auth login` in progress on the founder's side (answers Q-006). The Event Grid subscription for `Microsoft.Communication.IncomingCall` is created now even though no number is assigned to trigger it yet; it will simply receive nothing until a number exists. Q-002 itself (buying the number) remains OPEN and unblocked-M6 does not resolve it — the deploy can complete and even be smoke-tested before the number arrives.
**Why:** the founder judged that waiting on an open Azure support ticket with no ETA (Q-002) should not block standing up everything else the pilot needs — infra work, identity/role wiring, and a VoIP-only pipeline validation can all happen in parallel with the number purchase, since none of that work depends on which number eventually gets assigned. **Source:** founder, direct instruction, 2026-09-27. Q-003 (region: East US 2), Q-004 (system-assigned identity), Q-005 (verify JWT at deploy time), and Q-006 (`az login`/`azd auth login`, in progress) were already answered per D-018's 2026-09-27 status PR (#10) and remain the basis for this deploy. **A separate M6 implementation plan (units, tests, DoD) is written by the partner before any `azd`/Bicep work starts** — see the milestones doc's M6 section and STATUS.md §1 for the unit breakdown once written.
