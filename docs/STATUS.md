...
| U06 | Task 5 — expiry hooks | M2 | NEXT | `feat/tb-t05-expiry-hooks` | — | 0/4 | — | — |
...
**Unit tests passing overall:** 149 / ~250 (running total, `pytest --collect-only`). U01: 1 planned → 3; U02: 7 planned → 10; U03: 20 planned → 38; U04: 34 planned → 92 (four review rounds on the startup validator — see §2); U05: 5 planned → 5 (+1 regression test landed in U04's `test_bridge_config.py` for a `cso` finding, so the file-level total this unit touched is 149). Later units' "planned" counts in this table are the plan's original estimates and will likely grow the same way once reviewed.
