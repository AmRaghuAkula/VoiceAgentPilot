...
| U08 | Task 7 — call session | M3 | NEXT | `feat/tb-t07-call-session` | — | 0/31 | — | — |
...
**Unit tests passing overall:** 165 / ~250 (running total, `pytest --collect-only`). U01: 1 planned → 3; U02: 7 planned → 10; U03: 20 planned → 38; U04: 34 planned → 92 (four review rounds on the startup validator — see §2); U05: 5 planned → 5 (+1 regression test landed in U04's `test_bridge_config.py` for a `cso` finding, so the file-level total this unit touched is 149); U06: 4 planned → 4; U07: 11 planned → 12 (one regression test added for the interim_response deep-thaw bug found in review — see D-026). Later units' "planned" counts in this table are the plan's original estimates and will likely grow the same way once reviewed.
